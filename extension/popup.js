;(function () {
  var globalInputAppChromeExtension = {
    ...
    _getFormSettingLocalStoragePrefix: function () {
      var formprefix =
        'extension.' + this.getHostName() ? this.getHostName() : 'default'
    ...
    updateCacheTimer: function () {
      ...
      if (
        this.pagedata.action === 'display-cached-values' ||
        this.pagedata.action === 'connected-window'
      ) {
        that = this
        this.sendMessageToContent(
          ...
          function (message) {
            if (message) {
              ...
              if (cacheTTL > 500) {
                that.pagedata.cacheTimer = setTimeout(
                  that.updateCacheTimer.bind(that),
                  cacheTTL
                )
              }
            } else {
    ...
    getCustomApplicationControlConfig: function () {
      var applicationConfigString = localStorage.getItem(
        'iterative.globaliputapp.controlConfigs'
      )
      if (applicationConfigString) {
        try {
          return (applicatoonControlConfig = JSON.parse(
    ...
    isApplicationControlDisabled: function () {
      var applicationConfigString = localStorage.getItem(
        'iterative.globaliputapp.controlDisabled'
      )
      if (applicationConfigString) {
        return true
      }
    ...
    updateCustomApplicationControlConfig: function (newContent) {
      ...
      var customApplicationControlConfig =
        this.getCustomApplicationControlConfig()
      ...
      if (customApplicationControlConfig) {
        for (var i = 0; i < customApplicationControlConfig.length; i++) {
          if (
            notUpdated &&
            this.applicationConfigMatchHostName(
              customApplicationControlConfig[i],
              hostname
            )
          ) {
            if (newContent) {
            ...
            newCustomApplicationControlConfig.push(
              customApplicationControlConfig[i]
            )
          }
        }
    ...
    applicationConfigMatchHostName: function (
      ...
      if (applicationControlConfig.hostnames.type === 'single') {
        ...
        for (
          var k = 0;
          k < applicationControlConfig.hostnames.value.length;
          k++
        ) {
    ...
    selectApplicationControlConfig: function (
      hostname,
      applicationControlConfigs
    ) {
      for (var i = 0; i < applicationControlConfigs.length; i++) {
        if (
          this.applicationConfigMatchHostName(
            applicationControlConfigs[i],
            hostname
          )
        ) {
    ...
    getApplicationControlSettings: function () {
      ...
      var applicationControlSelector = {
        getConfig: function () {
          ...
          var customApplicationConfigString =
            chromeExtension.getCustomApplicationControlConfig()
          if (customApplicationConfigString) {
            var selectedApplicationControlConfig =
              chromeExtension.selectApplicationControlConfig(
                hostname,
                customApplicationConfigString
              )
          ...
          if (chromeExtension.applicationControlConfigs) {
            var selectedApplicationControlConfig =
              chromeExtension.selectApplicationControlConfig(
                hostname,
                chromeExtension.applicationControlConfigs
              )
            if (selectedApplicationControlConfig) {
              return {
                type: 'default',
    ...
    connectToGlobalInputApp: function (globalinputConfig) {
      ...
      this.pagedata.globalInputConnector =
        this.pagedata.globalInputApi.createMessageConnector() //Create the connector
    ...
    appendGlobalInputAppAvailable: function () {
      var element = this.createHTMLElement(
        'Scan the QR Code with <a href="https://globalinput.co.uk/" target="_blank">Global Input App</a>.'
      )
    ...
    setFormFieldValue: function (fieldId, newValue) {
      ...
      this.sendMessageToContent(
        'set-cache-field',
        { fieldId: fieldId, fieldValue: newValue },
        function (message) {
          //for the moment
        }
      )
    ...
    addFormElement: function (fieldId, formElement, inputContainer) {
      ...
      var onkeyup = function () {
        if (that.isSenderConnected()) {
          ...
          that.pagedata.globalInputConnector.sendInputMessage(
            value,
            this.fieldIndex
          )
        }
    ...
    onDocumentLoaded: function () {
      ;(this.applicationControlConfigs =
        globalInputApp_applicationControlConfigs),
        this.createContentContainer()
      chrome.runtime.onMessage.addListener(
        this.onContentMessageReceived.bind(this)
      )
      this.main()
    },
    ...
    saveFormFieldSettings: function (formSettings) {
      ...
      localStorage.setItem(
        this._getFormSettingLocalStoragePrefix(),
        fieldString
      )
    },
    getWidowFormSettings: function (loadDefaultSettings) {
      ...
      var fieldString = localStorage.getItem(
        this._getFormSettingLocalStoragePrefix()
      )
      if (fieldString) {
    ...
    saveGlobalInputSettings: function (globalInputSettings) {
      localStorage.setItem(
        'iterative.globaliputapp.url',
        globalInputSettings.url
      )
      localStorage.setItem(
        'iterative.globaliputapp.apikey',
        globalInputSettings.apikey
      )
    },
      ...
      var applicationControlEditor = {
        ...
        start: function () {
          ...
          if (chromeExtension.getHostName()) {
            ...
            chromeExtension.checkPageStatus(
              function (message) {
        ...
        showMain: function () {
          this.data.applicationControlSettings =
            chromeExtension.getApplicationControlSettings()

          if (
            !chromeExtension.isApplicationControlDisabled() &&
            this.data.applicationControlSettings
          ) {
            this.displayEditor()
          } else {
        ...
        getEditContent: function () {
          if (
            this.data.applicationControlSettings &&
            this.data.applicationControlSettings.applicationConfigs
          ) {
            return this.getObjectStringForm(
              this.data.applicationControlSettings.applicationConfigs
            )
          } else {
            return ''
        ...
        deleteRecord: function () {
          chromeExtension.deleteCustomApplicationControlConfig(
            this.data.hostname
          )
        ...
        saveEditContent: function () {
          ...
          try {
            ...
            if (
              !chromeExtension.applicationConfigMatchHostName(
                content,
                chromeExtension.getHostName()
              )
            ) {
              this.setMessageText(
                'hostname in the config should match the current url'
              )
              return
            }
            if (!content.forms) {
        ...
        enableApplicationControl: function () {
          ...
          if (!this.data.applicationControlSettings) {
            this.data.applicationControlSettings =
              this.createNewApplicationControlSetting()
          }
          this.displayEditor()
        },
        displayEditor: function () {
          ...
          chromeExtension.appendElement(
            chromeExtension.createHTMLElement(
              '<a href="https://github.com/global-input/chrome-extension/tree/master/application-controls" target="_blank">example settings</a>'
            )
          )
        },
    ...
    displayProxySettings: function () {
      ...
      this.appendElement(
        this.createHTMLElement(
          'The <a href="https://github.com/global-input/global-input-node" target="_blank">GIA WebSocket Server</a> is responsible for routing the encrypted messages between the extension and the <a href="https://globalinput.co.uk/" target="_blank">Global Input App</a>.'
        )
      )
    },
    displayWindowFormEditor: function () {
      ...
      var formEditorData = {
        ...
        deleteAField: function (fieldid) {
          this.modified = true
          this.formSettings.fields = this.formSettings.fields.filter(
            f => f.id !== fieldid
          )
          this.formSettings.isDefault = false
        },
        ...
        displayAddNewField: function () {
          ...
          var inputContainer = chromeExtension.createInputField(
            newFieldData.nameProperty
          )
          chromeExtension.appendElement(inputContainer)

          inputContainer = chromeExtension.createRadioButtons(
            newFieldData.linesProperty
          )
          chromeExtension.appendElement(inputContainer)
        ...
        appendFormEditorButtons: function () {
          if (this.modified && this.formSettings.fields.length) {
            ...
            var buttonContainer = chromeExtension.createButtons({
              buttons: [{ label: 'Cancel', onclick: this.cancelEdit }]
            })
            chromeExtension.appendElement(buttonContainer)
          }
        },
    ...
    requestContentFormFromContent () {
      ...
      if (applicationControlSettings && !this.isApplicationControlDisabled()) {
        request.applicationControlConfig =
          applicationControlSettings.applicationConfigs
      }
      var that = this
      this.sendMessageToContent('get-page-config', request, function (message) {
        ...
        var globalinputConfig =
          that.buildBasicGlobalInputConfig(globalInputSettings)
        if (message.status === 'success') {
          globalinputConfig.initData.form =
            that.buildContentGlobalInputForm(message)
    ...
    reconnectToWindowForm: function () {
      var globalInputSettings = this.getGlobalInputSettings()
      var globalinputConfig =
        this.buildBasicGlobalInputConfig(globalInputSettings)
    ...
    whenEmptyReplyReceived: function () {
      ...
      this.clearContent()
      var element = this.createHTMLElement(
        'Check to make sure that the URL on the web adress bar is correct, and then reload/refresh the page and try again. If the problem persist, you may temporarily use <a href="https://globalinput.co.uk/global-input-app/form-data-transfer" target="_blank">this web application</a> for the time being for this website.'
      )
      this.appendElement(element)
    },
    ...
    displayConnectedWindowForm: function () {
      ...
      this.appendElement(
        this.createHTMLElement(
          'You can now transfer content from/to mobile using the form above. You can copy and paste content across devices.'
        )
      )
          ...
          operations: {
            ...
            onInput: function (newValue) {
              ...
              that.sendMessageToContent(
                'set-form-field',
                { fieldId: field.id, fieldValue: newValue },
                function (message) {
      ...
      if (opts.clipboard) {
        ...
        copyButtonElement.onclick = function () {
          ...
          messageElement.innerText =
            'The ' + opts.id + ' is copied into the clipboard'
  ...
  document.addEventListener(
    'DOMContentLoaded',
    globalInputAppChromeExtension.onDocumentLoaded.bind(
      globalInputAppChromeExtension
    )
  )
})()
