...
GlobalInputMessageConnector=function(){_createClass(GlobalInputMessageConnector,[{key:"logError",value:function logError(
message,error){
if(error){
...
}},{key:"_connectToSocket",value:function _connectToSocket(
options){
...
}},{key:"buildPairingData",value:function buildPairingData()
...
buildPairingData:function buildPairingData(connector){var data=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};
var codedata=_extends({},data,{
...
decrypt=decrypt;exports.


basicGetURL=basicGetURL;function generatateRandomString(){var length=arguments.length>0&&arguments[0]!==undefined?arguments[0]:10;var randPassword=Array(length).fill("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz").map(function(x){return x[Math.floor(Math.random()*x.length)];}).join('');return randPassword;}function encrypt(content,password){return escape(CryptoJS.AES.encrypt(content,password).toString());}function decrypt(content,password){return CryptoJS.AES.decrypt(unescape(content),password).toString(CryptoJS.enc.Utf8);}function basicGetURL(url,onSuccess,onError){
